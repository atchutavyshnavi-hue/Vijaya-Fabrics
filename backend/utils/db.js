...
/* ---------- CRUD ---------- */
function getAllSarees({ category, subtype } = {}) {
  let list = readDb().sarees;
  if (category && category !== "all") list = list.filter((s) => s.category === category);
  if (subtype && subtype !== "all") list = list.filter((s) => s.subtype === subtype);
  return list;
}

function getSareeById(id) {
  return readDb().sarees.find((s) => s.id === Number(id));
}

function addSaree(data) {
  const db = readDb();
  const saree = { id: db.nextId, ...data };
  db.sarees.push(saree);
  db.nextId += 1;
  writeDb(db);
  return saree;
}

function updateSaree(id, updates) {
  const db = readDb();
  const idx = db.sarees.findIndex((s) => s.id === Number(id));
  if (idx === -1) return null;
  db.sarees[idx] = { ...db.sarees[idx], ...updates, id: db.sarees[idx].id };
  writeDb(db);
  return db.sarees[idx];
}

function deleteSaree(id) {
  const db = readDb();
  const before = db.sarees.length;
  db.sarees = db.sarees.filter((s) => s.id !== Number(id));
  writeDb(db);
  return db.sarees.length < before;
}
